...
h, w, _ = image.shape
image[:h//2, w//2:] = [255, 200, 255]  
cv2.imshow("Top-Right Colored (light pink)", image)
cv2.waitKey(0)
cv2.destroyAllWindows()

# 3. Brighten Bottom-Left Quarter
image = original_image.copy()
image[h//2:, :w//2] = cv2.add(image[h//2:, :w//2], 80)  # Increase brightness by 80
cv2.imshow("Bottom-Left Brightened", image)
cv2.waitKey(0)
cv2.destroyAllWindows()
